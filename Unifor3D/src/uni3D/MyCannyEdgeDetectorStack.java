...
public class MyCannyEdgeDetectorStack implements PlugIn {
	...
	public void run(String arg) {
		...
		ImageStack outStack = new ImageStack(sourceImage.getWidth(), sourceImage.getHeight());

		for (int i1 = 0; i1 < sourceImage.getStackSize(); i1++) {
			ImagePlus imp1 = MyStackUtils.imageFromStack(sourceImage, i1 + 1);
			imp1.show();
			MyLog.waitHere("immagine " + i1 + " / " + sourceImage.getStackSize());

			Undo.setup(Undo.TYPE_CONVERSION, imp1);

			process();

			ImageProcessor ip1 = new FloatProcessor(imp1.getWidth(), imp1.getHeight(), data);
			ip1 = ip1.convertToByte(false);
			if (!Prefs.blackBackground)
				ip1.invertLut();
			sourceImage.setProcessor(ip1);
			outStack.addSlice(ip1);
		}
		ImagePlus impOut = new ImagePlus("cannyStack", outStack);
	}
	...
	public ImagePlus process(ImagePlus imp) {
		...
		ImageStack outStack = new ImageStack(imp.getWidth(), imp.getHeight());

		for (int i1 = 0; i1 < imp.getStackSize(); i1++) {
			ImagePlus imp1 = MyStackUtils.imageFromStack(imp, i1 + 1);
			// imp1.show();
			// MyLog.waitHere("immagine " + i1 + " / " + imp.getStackSize());
			sourceImage = imp1;

			process();
			ip1 = new FloatProcessor(imp1.getWidth(), imp1.getHeight(), data);
			ip1 = ip1.convertToByte(false);
			outStack.addSlice(ip1);
